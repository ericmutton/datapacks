execute if entity @a[tag=delhome,scores={homes.max=2..},limit=1] run scoreboard players operation #current ordinal = #current delhome
execute if entity @a[tag=delhome,scores={homes.max=2..},limit=1] run function server:ordinals
tellraw @a[tag=delhome,scores={homes.max=2..},limit=1] [{"text":"You have reset your ","color":"gray"},{"score":{"name":"#current","objective":"delhome"}},{"nbt":"text.ordinal","storage":"server"}," ",{"selector":"@s"}," home."]
